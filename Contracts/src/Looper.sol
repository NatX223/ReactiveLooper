...
contract TokenSupplier is AbstractCallback, Ownable {
    ...
    function calculateSafeBorrowAmount() public view returns (uint256 borrowAmount) {
        
        // 1. Fetch current position data from Aave Pool
        (, , uint256 availableBorrowsBase, , , ) = Pool.getUserAccountData(address(this));
        // uint256 availableBorrowsBase = availableBorrowsBase;

        // 2. Check if there is anything available to borrow
        if (availableBorrowsBase == 0) {
            return 0;
        }

        // 3. Apply the Safety Buffer
        // availableBorrowsBase is in 8 decimals (e.g., 100.00 * 1e8)
        uint256 safeBorrowsBase = (availableBorrowsBase * SAFETY_FACTOR_PERCENT) / DENOMINATOR;

        // 4. Get the current price of the borrow token (USDC) in Base Currency (USD)
        // Price oracle returns 8 decimals for USDC (usually 1.00000000 * 1e8)
        ...
        // 5. Convert Base Value (USD) into Borrow Token Units (USDC)
        
        // The formula for token amount: (Value in Base * 10^TokenDecimals) / (Price in Base * 10^BaseDecimals)
        // Since both Price and Value are normalized to 8 decimals in Aave V3:
        
        // a. Convert safeBorrowsBase value to the final borrow token units
        // safeBorrowsBase (8 decimals) -> safeBorrowsBase / 10^(8-6) = safeBorrowsBase / 100
        uint256 conversionFactor = 10**(BASE_CURRENCY_DECIMALS - BORROW_TOKEN_DECIMALS); // 100

        // The final token amount is: (safeBorrowsBase / borrowTokenPriceBase) * (10^BorrowDecimals)
        // Since Price and Value are in 8 decimals, we simplify the scaling:
    ...
    function callback(
        ...
        if (operation(_operation) == operation.supply) {
            ...
            // Pool.borrow(borrowToken, , 2, 0, address(this));
